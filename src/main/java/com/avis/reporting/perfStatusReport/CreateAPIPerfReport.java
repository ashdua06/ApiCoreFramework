package com.avis.reporting.perfStatusReport;
...
public class CreateAPIPerfReport {
    ...
    public static void createHTMLReport(String apiPerfReportFileName, String apiPerfReportTitle) {
        File destFolderPath = new File(AvisConstants.REPORT_BASE_PATH + "APIPerf");
        try {
        ...
        try {
            ...
            PrintHtml.println(
                    "<tr align='center'><th>API Base Path </th><th>Total Number of Hits </th><th>Average Response Time </th><th>Max Response Time</th><th>Minimum Response Time</th>" +
                            "</tr>");
            for (String apiBasePath : AvisConstants.API_STATUS_TIME_DTOMAP.keySet()) {
                Long totalHits = AvisConstants.API_STATUS_TIME_DTOMAP.get(apiBasePath).getTotalHits();
                Long averageResponseTime = AvisConstants.API_STATUS_TIME_DTOMAP.get(apiBasePath).getAverageResponseTime();
                Long maxTime = AvisConstants.API_STATUS_TIME_DTOMAP.get(apiBasePath).getMaxTime();
                Long minTime = AvisConstants.API_STATUS_TIME_DTOMAP.get(apiBasePath).getMinTime();
    ...
    public void captureAPIStatusTimeDTO(String basePath, Long timeInMilliSeconds) {
        if (AvisConstants.API_STATUS_TIME_DTOMAP.containsKey(basePath)) {
            APIStatusTimeDTO apiStatusTimeDTO = AvisConstants.API_STATUS_TIME_DTOMAP.get(basePath);
            apiStatusTimeDTO.setResponseTime(timeInMilliSeconds);
        } else {
            APIStatusTimeDTO apiStatusTimeDTO = new APIStatusTimeDTO(basePath);
            apiStatusTimeDTO.setResponseTime(timeInMilliSeconds);
            AvisConstants.API_STATUS_TIME_DTOMAP.put(basePath, apiStatusTimeDTO);
        }
